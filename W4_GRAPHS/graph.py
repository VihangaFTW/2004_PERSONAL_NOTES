...
from disjointed_set import DisjointedSet
...
class Graph:
    ...
    '''
    #? * is a special python syntax where every keyword argument after the * should be explicitly stated upon object instantiation
    
    def __init__(self, *,vertices: list[Vertex] | None = None, init_size:int = 0 ) -> None:
        # if vertices are not provided, initialize an empty array for vertices based on init_size
        if not vertices and init_size >0:
            self.vertices = [None] * init_size
            self.size = 0
        # otherwise, initialize vertices array as normal
        elif vertices:
            self.vertices: list[None|Vertex] = [None] * len(vertices)
            for i in range(len(vertices)):
                self.vertices[i] = vertices[i]
            self.size = len(vertices)
        else:
            raise Exception("Graph initialization size not declared!")
    ...
    def add_vertex(self,vertex: Vertex) -> None:
        self.size += 1
        if self.size >= len(self.vertices):
            self._resize_vertices_array
        self.vertices[self.size] = vertex
    
    def _resize_vertices_array(self)-> None:
        '''
        Doubles the vertices array size if it becomes full.
        '''
        new_arr = [None] * len(self.vertices) * 2
        for i in range(len(self.vertices)):
            new_arr[i] = self.vertices[i]
        self.vertices = new_arr
...
#!=============================================================================================
    ...
    def find_MST_Prim(self, start_vertex: Vertex) -> str:
        ...
        : returns a formatted string showing a set of the edges making up the minimum spanning tree. Answers might not be unique.
        : time comp:- O(V Log V + E log V) = O(E log V) when E>= V
        : space comp: aux is O(V) as we need to store all the vertices in the heap.
                      total is O(V).
        """
        ...
        while discovered:
            ...
            if current_vertex.edge:
                mst_edges.append(str(current_vertex.edge))
        ...
        return f'Minimum Spanning Tree consists of these edges {mst_edges}' 
    
    
    def find_MST_Kruskal(self) -> str:
        '''
        Method that finds the edges that form a mimimum spanning tree from the graph using Kruskal's algorithm 
        which is based on the union of trees using a Disjointed Set adt.
        : return : a list of edges (in order) that forms the minimum spanning tree
        : time complexity: O(E log E) in the worst case where V are the number of vertices and E are the edges in the graph.
        '''
        
        self._reset_attributes()
        # initialize a disjointe set to combine the mst 
        d_set = DisjointedSet(len(self.vertices))
        # initialize an array to store the edges in the graph
        edges: list[Edge|None] = []

        # retrieve all edges
        for i in range(len(self.vertices)): #? O(V+E)
            d_set.add(self.vertices[i])
        ...
        mst_edges = []
        for edge in edges: #? O(E log V)
            u = edge.u
            v = edge.v
            if d_set.union(u,v): #? log V
                mst_edges.append(str(edge))
...
if __name__ == "__main__":
    ...
    # v4 = Vertex("D")
    # v5 = Vertex("E")
    # graph2 = Graph(vertices=[v1,v2,v3,v4,v5])
    ...
    graph = Graph(vertices=[v1, v2, v3, v4, v5])
    graph.add_undirected_edge(v1, v2, 3)
    graph.add_undirected_edge(v1, v3, 2)
    graph.add_undirected_edge(v2, v3, 1)
    graph.add_undirected_edge(v2, v4, 4)
    graph.add_undirected_edge(v3, v4, 5)
    graph.add_undirected_edge(v4, v5, 6)
    graph.add_undirected_edge(v3, v5, 7)
    print(graph.find_MST_Prim(v1))
    print(graph.find_MST_Kruskal())
    

    

    #* testing DisjointedSet
    # v1 = Vertex("1")
    # v2 = Vertex("2")
    # v3 = Vertex("3")
    # v4 = Vertex("4")
    # v5 = Vertex("5")
    # my_set = DisjointedSet(5)
    # my_set.add(v1)
    # my_set.add(v2)
    # my_set.add(v3)
    # my_set.add(v4)
